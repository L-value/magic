...
* 项目名称：ImageLoader   
* 类名称：ImageDownloader   
* 类描述：异步下载的核心类，保存图片到手机缓存，将图片加入LruCache中等等   首先我们需要重写sizeOf(String key, Bitmap value)来计算图片的大小，默认返回图片的数量
downloadImage(final String url, final onImageLoaderListener listener)先去LruCache查看Image,没有再去手机缓存中查看，在没有则开启线程下载，这里
我们提供了一个回调接口，回调方法中我们将Bitmap和图片Url作为参数，String subUrl = url.replaceAll("[^\\w]", "") 我在代码中注释写的比较清楚
showCacheBitmap(String url)方法，此方法在Adapter中的getView()当中调用，如果getView()中不调用此方法试试你就知道效果了
* 创建人：fuli   
* 创建时间：2015年7月26日 上午9:12:22   
* 修改人：fuli   
* 修改时间：2015年7月26日 上午9:12:22   
* 修改备注：   
* @version    
*    
*/
